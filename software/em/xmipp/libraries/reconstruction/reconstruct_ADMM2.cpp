...
#include "reconstruct_ADMM2.h"
#include "symmetrize.h"
#include <data/metadata_extension.h>

#define SYMMETRIZE_PROJECTIONS

ProgReconsADMM2::ProgReconsADMM2(): XmippProgram()
{
	rank=0;
	Nprocs=1;
}

void ProgReconsADMM2::defineParams()
{
...
void ProgReconsADMM2::readParams()
{
	fnIn=getParam("-i");
...
void ProgReconsADMM2::produceSideInfo()
{
	// Read input images
	mdIn.read(fnIn);
	...
	mu*=Nimgs/XSIZE(CHtb())*CHtb_norm;
	lambda*=Nimgs/XSIZE(CHtb())*CHtb_norm;
...
void ProgReconsADMM2::show()
{

}

void ProgReconsADMM2::run()
{
	produceSideInfo();
	if (rank==0)
	{
...
void ProgReconsADMM2::constructHtb()
{
...
void ProgReconsADMM2::project(double rot, double tilt, double psi, MultidimArray<double> &P, bool adjoint, double weight)
{
	Matrix2D<double> E;
...
void ProgReconsADMM2::project(const Matrix1D<double> &r1, const Matrix1D<double> &r2, MultidimArray<double> &P, bool adjoint, double weight)
{
...
void ProgReconsADMM2::computeHtKH(MultidimArray<double> &kernelV)
{
...
void addGradientTerm2(double mu, AdmmKernel2 &kernel, MultidimArray<double> &L, FourierTransformer &transformer,
...
void ProgReconsADMM2::addRegularizationTerms()
{
	MultidimArray<double> L;
	...
	if (mu>0)
	{
		addGradientTerm2(mu, kernel,L,transformer,fourierKernelV, 'x');
		addGradientTerm2(mu, kernel,L,transformer,fourierKernelV, 'y');
		addGradientTerm2(mu, kernel,L,transformer,fourierKernelV, 'z');
	}
...
void ProgReconsADMM2::applyKernel3D(MultidimArray<double> &x, MultidimArray<double> &AtAx)
{
...
void ProgReconsADMM2::applyLFilter(MultidimArray< std::complex<double> > &fourierL, bool adjoint)
{
...
void ProgReconsADMM2::applyLtFilter(MultidimArray< std::complex<double> > &fourierL, MultidimArray<double> &u, MultidimArray<double> &d)
{
...
void ProgReconsADMM2::applyConjugateGradient()
{
...
void ProgReconsADMM2::doPOCSProjection()
{
	if (applyMask)
...
//	std::cout << "Threshold=" << threshold << std::endl;
    ...
    u = ux*ux+uy*uy+uz*uz;
    u.selfSQRT();
    FOR_ALL_ELEMENTS_IN_ARRAY3D(u)
    {
    	if(A3D_ELEM(u,k,i,j)>threshold)
    	{
    		A3D_ELEM(ux,k,i,j)=A3D_ELEM(ux,k,i,j)*(A3D_ELEM(u,k,i,j)-threshold)/A3D_ELEM(u,k,i,j);
    		A3D_ELEM(uy,k,i,j)=A3D_ELEM(uy,k,i,j)*(A3D_ELEM(u,k,i,j)-threshold)/A3D_ELEM(u,k,i,j);
    		A3D_ELEM(uz,k,i,j)=A3D_ELEM(uz,k,i,j)*(A3D_ELEM(u,k,i,j)-threshold)/A3D_ELEM(u,k,i,j);
    	}
    	else
    	{
    		A3D_ELEM(ux,k,i,j)=0;
...
void ProgReconsADMM2::produceVolume()
{
	Vk() = Ck();
}

void AdmmKernel2::initializeKernel(double _alpha, double a, double _projectionStep)
{
...
double AdmmKernel2::projectionValueAt(double u, double v) {
...
void AdmmKernel2::convolveKernelWithItself(double _autocorrStep)
{
...
void AdmmKernel2::getKernelAutocorrelation(MultidimArray<double> &autocorrelation)
{
...
void AdmmKernel2::applyCTFToKernelAutocorrelation(CTFDescription &ctf, double Ts, MultidimArray<double> &autocorrelationWithCTF)
{
...
void AdmmKernel2::computeGradient(MultidimArray<double> &gradient, char direction, bool adjoint)
{
	gradient.initZeros();
	for (int k=-1; k<=1; ++k)
	{
		...
		switch (direction)
		{
		case 'x': A3D_ELEM(gradient,0,0,k)=value*k; break;
		case 'y': A3D_ELEM(gradient,0,k,0)=value*k; break;
		case 'z': A3D_ELEM(gradient,k,0,0)=value*k; break;
		}
	}
}

void AdmmKernel2::computeKernel3D(MultidimArray<double> &kernel)
{
