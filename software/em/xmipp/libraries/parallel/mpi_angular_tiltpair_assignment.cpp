...
#include "mpi_angular_tiltpair_assignment.h"

MpiProgAngularTiltPairAssignment::MpiProgAngularTiltPairAssignment()
{
	node=NULL;
}
...
void MpiProgAngularTiltPairAssignment::read(int argc, char** argv)
{
    node = new MpiNode(argc, argv);
   	rank = node->rank;
   	Nprocessors = node->size;

   	ProgAngularTiltPairAssignment::read(argc, (const char **)argv);
}

void MpiProgAngularTiltPairAssignment::synchronize()
{
	node->barrierWait();
}
